...
"""
Setup helper for agent-zot.

This script provides utilities to automatically configure agent-zot
...
def find_executable():
    """Find the full path to the agent-zot executable."""
    ...
    if exe_path:
        print(f"Found agent-zot in PATH at: {exe_path}")
        return exe_path
    ...
    for path in potential_paths:
        if path.exists() and os.access(path, os.X_OK):
            print(f"Found agent-zot at: {path}")
            return str(path)
    
    # If still not found, search in common directories
    print("Searching for agent-zot in common locations...")
    try:
        # On Unix-like systems, try using the 'find' command
        if sys.platform != 'win32':
            import subprocess
            result = subprocess.run(
                ["find", os.path.expanduser("~"), "-name", "agent-zot", "-type", "f", "-executable"],
                capture_output=True, text=True, timeout=10
            )
            paths = result.stdout.strip().split('\n')
            if paths and paths[0]:
                print(f"Found agent-zot at {paths[0]}")
                return paths[0]
    except Exception as e:
        print(f"Error searching for agent-zot: {e}")
    
    print("Warning: Could not find agent-zot executable.")
    print("Make sure agent-zot is installed and in your PATH.")
    return None
...
def setup_semantic_search(existing_semantic_config: dict = None, semantic_config_only_arg: bool = False) -> dict:
    ...
    print("1. Manual - Update only when you run 'agent-zot update-db'")
...
def update_claude_config(config_path, zotero_mcp_path, local=True, api_key=None, library_id=None, library_type="user", semantic_config=None):
    """Update Claude Desktop config to add agent-zot."""
...
def _write_standalone_config(local: bool, api_key: str, library_id: str, library_type: str, semantic_config: dict, no_claude: bool = False) -> Path:
    ...
    cfg_dir = Path.home() / ".config" / "agent-zot"
...
def main(cli_args=None):
    ...
    parser = argparse.ArgumentParser(description="Configure agent-zot for Claude Desktop")
    ...
    semantic_config_dir = Path.home() / ".config" / "agent-zot"
    ...
    if args.semantic_config_only:
        ...
        if semantic_config_changed:
            if save_semantic_search_config(new_semantic_config, semantic_config_path):
                ...
                print("\nTo initialize the database, run: agent-zot update-db")
                return 0
            else:
    ...
    # Find agent-zot executable
    exe_path = find_executable()
    if not exe_path:
        print("Error: Could not find agent-zot executable.")
        return 1
    print(f"Using agent-zot at: {exe_path}")
    ...
    try:
        if args.no_claude:
            ...
            if semantic_config_changed:
                ...
                print("  agent-zot update-db --force-rebuild")
            return 0
        else:
            ...
            if updated_config_path:
                ...
                if semantic_config_changed:
                    ...
                    print("- To change the configuration, run: agent-zot setup --semantic-config-only")
                    print("- The config file is located at: ~/.config/agent-zot/config.json")
                    print("- You may need to rebuild your database: agent-zot update-db --force-rebuild")
                else:
                    print("\nSemantic Search:")
                    print("- To update the database, run: agent-zot update-db")
