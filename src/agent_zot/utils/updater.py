"""
Update functionality for agent-zot.
...
def detect_installation_method() -> str:
    """
    Detect how agent-zot was originally installed.
...
def is_pipx_installation() -> bool:
    """Check if agent-zot was installed via pipx."""
    try:
        ...
        if result.returncode == 0:
            return "agent-zot" in result.stdout
...
def get_current_version() -> Optional[str]:
    """Get the currently installed version of agent-zot."""
    try:
        ...
        try:
            result = subprocess.run(
                [sys.executable, "-m", "pip", "show", "agent-zot"],
                capture_output=True,
                text=True,
                timeout=10
            )
...
def get_latest_version() -> Optional[str]:
    ...
    try:
        response = requests.get(
            "https://api.github.com/repos/54yyyu/agent-zot/releases/latest",
            timeout=10
        )
...
def backup_configurations() -> Path:
    ...
    semantic_config_path = Path.home() / ".config" / "agent-zot" / "config.json"
    ...
    chroma_db_path = Path.home() / ".config" / "agent-zot" / "chroma_db"
...
def update_via_method(method: str, force: bool = False) -> Tuple[bool, str]:
    """
    Update agent-zot using the specified method.
    ...
    """
    repo_url = "git+https://github.com/54yyyu/agent-zot.git"
...
def update_zotero_mcp(check_only: bool = False, 
    ...
    """
    Main update function for agent-zot.
    ...
    # Perform update
    print(f"Updating agent-zot using {detected_method}...")
    try:
