...
def update_search_database(
    ...
    *,
    ...
    Args:
        ...
        modified_since: Optional ISO date filter (e.g., "2025-11-01"). Only process items modified after this date (Phase 4)
        retry_failed_only: Set to True to retry only previously failed items (Phase 4)
        cancel_flag: Optional dict with 'cancel' key for graceful cancellation (Phase 4)
        ctx: MCP context
    ...
    """
    try:
        # Check for Phase 4 features (not yet implemented)
        if modified_since is not None:
            return """⚠️  **Phase 4 Feature Not Yet Implemented**

**Requested**: Filter by modification date (`modified_since`)

**Status**: This feature requires implementing:
1. Date-based filtering in Zotero query layer
2. SQL WHERE clause for `dateModified` field
3. ISO date parsing and validation

**Current Workaround**: Use `limit` parameter for testing:
```
zot_manage_database("test on 10 papers")
```

**Full implementation**: Planned for Phase 4 completion.
"""

        if retry_failed_only:
            return """⚠️  **Phase 4 Feature Not Yet Implemented**

**Requested**: Retry only failed items (`retry_failed_only`)

**Status**: This feature requires implementing:
1. SQLite cache for tracking failed item keys
2. Error categorization (parse failures vs. API errors)
3. Retry logic with exponential backoff

**Current Workaround**: Run full update - it's idempotent and skips already-processed items.
...
**Requested**: Graceful cancellation (`cancel_flag`)

**Status**: This feature requires implementing:
1. Shared state mechanism for cancel signal
2. Batch-level check in update loop
3. Clean shutdown of worker processes

**Current Workaround**: Kill the MCP server process (it will auto-restart):
```
ps aux | grep "agent-zot serve"
kill <PID>
```
...
# Consolidates update, backup, restore, inspect, and monitoring operations
# into one elegant natural language interface
...
@mcp.tool(
    ...
    description="🔧 UNIFIED - Manage Qdrant and Neo4j databases with natural language - update, backup, restore, inspect, and monitor using simple commands",
    annotations={
        "readOnlyHint": False,
