...
def load_standalone_env_vars():
    """Load environment variables from standalone config (~/.config/agent-zot/config.json)."""
    try:
        from pathlib import Path
        cfg_path = Path.home() / ".config" / "agent-zot" / "config.json"
...
def main():
    ...
    # Setup command
    setup_parser = subparsers.add_parser("setup", help="Configure agent-zot (Claude Desktop or standalone)")
    ...
    if args.command == "version":
        from agent_zot._version import __version__
        ...
        try:
            ...
            if "agent-zot" in result.stdout:
                print("  Installation method: uv tool")
            else:
                # Check pip
                result = subprocess.run([sys.executable, "-m", "pip", "show", "agent-zot"], 
        ...
        if config_path.exists():
            ...
            print("  💡 Run 'agent-zot setup' to configure semantic search")
        
        sys.exit(0)
    
    elif args.command == "setup":
        from agent_zot.utils.setup import main as setup_main
        ...
        try:
            ...
            if args.check_only:
                ...
                if result.get('success'):
                    ...
                    print("• Run 'agent-zot version' to verify the update")
                else:
                    print("❌ Update failed!")
