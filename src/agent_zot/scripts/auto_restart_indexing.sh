#!/bin/bash
# Auto-restart watchdog for agent-zot indexing
...
VENV_PATH="$HOME/toolboxes/agent-zot-env"
CHECK_INTERVAL=60  # Check every 60 seconds

echo "Starting auto-restart watchdog for agent-zot indexing..."
...
while true; do
    ...
    if pgrep -f "agent-zot update-db --fulltext" > /dev/null; then
        ...
        tmux new-session -d -s "$TMUX_SESSION" \
            "source $VENV_PATH/bin/activate && agent-zot update-db --fulltext 2>&1 | tee -a $LOG_FILE"
